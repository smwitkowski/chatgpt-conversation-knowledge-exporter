...
def consolidate_project(
    ...
    console.print(f"[bold]Processing {len(conversation_dirs)} conversations[/bold]")
    ...
    console.print(f"[green]Writing consolidated files to {project_dir}[/green]")
    ...
    if include_docs and docs_dir.exists():
        ...
        if md_files:
            console.print(f"[dim]Concatenating {len(md_files)} markdown docs[/dim]")
            _concat_markdown_files(md_files, project_dir / "docs_concat.md")

        if adr_files:
            console.print(f"[dim]Concatenating {len(adr_files)} ADR files[/dim]")
